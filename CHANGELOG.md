...
### Added
- `src/schema.py` — `VisualAssetConfig.prompt_speeches` — new optional field to assign per-scene speech text to each prompt/image. When provided, each visual is displayed for a duration proportional to its word count, keeping audio and visuals in sync. The combined text should match `speech_content` exactly.
- `src/orchestrator.py` — `_compute_visual_durations()` — calculates per-visual display durations from `prompt_speeches` word counts. Falls back to equal-time distribution if `prompt_speeches` is absent or count mismatches.
- `src/assembler_adapter.py` — `visual_durations` parameter added to `local_moviepy_assemble()` and `_local_moviepy_assemble()`. When provided, each visual clip is built with its own slot duration instead of `total_duration / n_visuals`.
- `src/backends/native_assembler_backend.py` — `visual_durations` forwarded through `assemble()` to the underlying moviepy assembler.
- `TROUBLESHOOTING.md` — detailed entry covering image generation hang on cloud infrastructure: false Pollinations ban via IP range detection, per-image timeout with automatic fallover, orphaned `.generation.lock` causing blocked subsequent runs, and workarounds.

### Changed
- `src/image_providers/cloud_detection.py` — `BannedProviders.CLOUD_PROVIDER_BANS` cleared. Pollinations was incorrectly listed as banned on AWS/DigitalOcean/Hetzner/etc.; it responds successfully from those IPs. Provider is now attempted and fails naturally if unavailable, rather than being skipped preemptively.
- `src/image_providers/manager.py` — `generate_image()` now wraps each provider attempt in a `ThreadPoolExecutor` future with a configurable `per_image_timeout` (default 120s). If a provider exceeds the timeout, execution falls over to the next provider immediately instead of blocking the entire batch indefinitely.
- `src/image_providers/manager.py` — `generate_images_batch()` now renames each generated file to include a zero-padded sequence index prefix (`001_`, `002_`, …). This ensures alphabetical sort order on disk matches generation order, eliminating visual disorder when browsing output in file explorers or scripts that sort by filename.

### Fixed
- `src/ui.py` — uploaded files in `IMAGE_SEQUENCE`/`MEDIA_SEQUENCE` mode are now sorted by filename before building the `uploaded_images` dict. Previously the order was browser/OS-dependent (whatever order `st.file_uploader` returned), making multi-file uploads non-deterministic. Sorting alphabetically by `f.name` gives the user explicit control via numeric filename prefixes (e.g. `01_scene.jpg`, `02_scene.jpg`).
- `src/image_providers/pexels.py` — removed. The file contained a broken `PexelsProvider` implementation that used non-existent `ProviderResult` fields (`image_url`, `source`) and would have raised `TypeError` at runtime. `ImageEngine` enum never included `pexels` so it was unreachable from config anyway, but the dead import in `registry.py` was causing a latent crash risk.
- `src/image_providers/registry.py` — removed Pexels `ProviderSpec` registration and its import. The registry now only lists the five supported providers: Cloudflare, SiliconFlow, Pollinations, HuggingFace (FLUX + SDXL), and Picsum.
- `src/image_adapter.py` — extended the unsupported-engine guard to also reject `"pixabay"` explicitly, matching what `FREE_IMAGE_PROVIDERS.md` documents.
- `src/lock_service.py` — removed dead `try/except NameError` guards around `_LOCK_FDS` in both `acquire_background_lock()` and `release_background_lock()`. The dict is initialized at module level so those branches could never fire; they were misleading noise suggesting a real edge case that doesn't exist. The `global _LOCK_FDS` declarations were also removed since neither function rebinds the name.
- `src/image_providers/manager.py` — silent `OSError` swallow on sequence-index rename replaced with a `logger.warning`. If the rename fails (e.g. cross-device move, permissions), the original path is kept and the failure is now visible in logs instead of disappearing silently.
- `src/image_providers/registry.py` — added `reset_registry_for_testing()` function that clears the global `_registry` singleton under the lock. Intended for test isolation: call it in `setUp` / a pytest fixture to prevent registry state leaking between tests. Also updated `get_provider_registry()` docstring to mark the global accessor as deprecated for new code, pointing toward explicit `ProviderRegistry()` construction + DI as the preferred path.

### Known Limitations
- `src/visual_service.py` — When using `IMAGE_SEQUENCE` or `MEDIA_SEQUENCE` in the UI with both typed manual paths and drag-and-drop uploads, the final sequence is always `[manual paths] + [uploaded files]`. Uploads are appended after manual paths regardless of the order you added them. If the interleaving order matters, use only one source at a time.
...
- `src/config_loader.py` — added support for config path overrides via both an explicit `path` parameter to `load()` and a `CONFIG_PATH` environment variable; cache is now keyed by config file path (no race conditions when multiple threads use different config paths simultaneously).
- `src/image_adapter.py` — removed module-level `dotenv.load_dotenv`; `generate_from_prompts` now accepts an optional `provider_manager` parameter; `_try_native_image_generation` now accepts an optional `provider_manager` parameter; replaced global `_provider_manager` singleton with `_get_fresh_provider_manager()` that creates a new, clean ProviderManager for each video generation (prevents cross-contamination between video A and video B).
- `TROUBLESHOOTING.md` — added section about testing philosophy (mocks vs real tests) to address user concerns about mock-based tests not checking real code, and moved background music entry to new "Resolved Issues" section.
...
- `src/orchestrator.py` — refactored `VideoOrchestrator` to be a thin facade using the new specialized services; maintained full backward compatibility while reducing complexity; all private passthrough methods now emit FutureWarning to guide users to the new service classes.
- `src/visual_service.py` — fixed TypeError by only passing `provider_manager` to the default `image_adapter.generate_from_prompts`, not to gateway-provided `generate_from_prompts` callables; improved argument type checking to only swallow TypeError for signature mismatches, not other errors, with DeprecationWarning for old positional calling style.
- `src/video_gateway.py` — updated `GenerateFromPromptsCallable` Protocol to include optional `provider_manager: Optional[Any] = None` and `provider_registry: Optional[Any] = None` parameters for future extensibility.
- `src/duration_service.py` — new `DurationService` functions `probe_audio_duration()` and `resolve_total_duration()` extracted from `orchestrator.py` for better testability and reusability.
- `src/lock_service.py` — new `LockService` functions `acquire_background_lock()` and `release_background_lock()` extracted from `main.py` for better testability and reusability.
- `tests/test_duration_service.py` — dedicated tests for duration resolution with ffprobe and moviepy fallback.
- `tests/test_lock_service.py` — dedicated tests for lock acquisition/release.
- `config/nothing_song_background.yaml` — config for Depeche Mode "Nothing" video with background music.
- `src/schema.py` — `VideoConfiguration.subtitle_segments` — new optional field to accept explicit timed subtitle segments with `text`, `start`, and `end` (float seconds).
- `tests/test_video_gateway.py` — new test `test_orchestrator_forwards_provider_di_to_gateway_generate` to verify that provider manager and provider registry are correctly forwarded to gateway callables.
...
- `src/image_providers/manager.py` — detects cloud infrastructure at initialization and logs which providers are banned; accepts optional `metrics_collector` parameter for performance tracking; `generate_image()` method now tracks provider attempt timing and reports to metrics; generates informative error messages distinguishing between rate-limiting and IP-blocking; now reuses a single shared `ThreadPoolExecutor` across all image generations (fixes thread pool leak).
...
- `src/orchestrator.py` — when `background_music` is provided, the source file is copied into `workspace/audio` and the workspace-local copy is used for assembly; invalid paths raise `FileNotFoundError` before video assembly starts; fixed step numbering in `create_video()` log statements (step 8 is now cleanup, not step 7).
...
- `src/duration_service.py` — updated `resolve_total_duration()` to accept optional `audio_path` and `background_music_path` for duration resolution when speech audio is not provided.
- `src/orchestrator.py` — refactored to prepare background music first, then resolve duration using speech audio or background music as fallback.
- `src/assembly_service.py` — updated `generate_subtitle_segments()` to use `config.subtitle_segments` if provided.
- `src/assembler_adapter.py` — updated to handle `audio_path` being `None`, using background music only when no speech audio is present.
- `src/backends/native_assembler_backend.py` — updated `assemble()` method to accept optional `audio_path`.
- `src/subtitle_renderer.py` — updated to respect existing newlines, increase subtitle line limit from 2 to 5, update warning messages, and fix empty text handling.
- `tests/test_subtitle_renderer.py` — updated tests to match increased subtitle line limit and new behavior.
- `src/image_providers/registry.py` — added `_registry_lock` (threading.Lock) with double-checked locking pattern to protect global `_registry` singleton from race conditions during concurrent initialization.

### Fixed
- `src/video_gateway.py` / `src/visual_service.py` — fixed provider DI forwarding for gateway-supplied `generate_images_from_prompts`; `provider_manager` and `provider_registry` now reach gateway callables the same way they do the adapter fallback path.
- `src/image_providers/manager.py` — fixed a thread pool leak by reusing a shared `ThreadPoolExecutor` across image attempts and exposing a clean `shutdown()` path instead of creating a new pool per image with `shutdown(wait=False)`.
...
- `tests/test_orchestrator.py` — added regression coverage for background music workspace relocation and invalid background music path handling, and `test_burn_subtitles_raises_when_backend_is_missing` to ensure clear error when subtitles are enabled but no backend is available.
...
- `src/assembly_service.py` — restored the missing check for subtitle backend availability in `assemble_and_burn_video()`; now raises `RuntimeError` with clear message when subtitles are enabled but no backend is configured (regression from service extraction).
- `src/config_loader.py` — fixed race condition when multiple threads use different config paths; cache is now a dict keyed by config file path string instead of single global entry, removed `_current_config_path` entirely to eliminate state leakage between threads.
