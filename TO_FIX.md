...
2. Decouple `ui.py` from CLI internals (medium)
  - Remove `src.ui`'s imports of private functions from `src.main` and expose lock coordination through a shared boundary/service.

### Medium priority

3. Extract duration resolution into its own testable module (medium)
...
4. Clean up VideoContext (medium)
...
5. Replace subtitle timing estimation with timestamp-based alignment (hard)
